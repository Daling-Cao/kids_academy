...
interface DoorProps {
  key?: React.Key;
  project: { title: string; coverImage: string; state: string;[key: string]: any };
  index: number;
  onClick: () => void | Promise<void>;
}

export default function Door({ project, index, onClick }: DoorProps) {
  ...
  return (
    <div
      className={`relative w-48 h-72 rounded-t-full shadow-xl cursor-pointer transition-transform hover:scale-105 ${state === 'locked' ? 'opacity-80' : ''
        }`}
      onClick={onClick}
    >
      {/* Door Frame */}
